			...
			if (args.length == 0) {
				...
				if (world.getDifficulty() != Difficulty.HARD || world.getGameRuleValue(GameRule.NATURAL_REGENERATION) || !world.isHardcore()) {
					...
					if (!world.isHardcore()) {
						...
						Bukkit.broadcastMessage(ChatColor.DARK_RED + "Le serveur doit redémarrer pour changer votre mode de jeu");
					...
					Bukkit.broadcastMessage(ChatColor.GOLD + "Votre mode de jeu a été mis à jour");
					Bukkit.broadcastMessage(ChatColor.YELLOW + "Le scénario UHC est maintenant activé. Ne perdez plus de vie !");
