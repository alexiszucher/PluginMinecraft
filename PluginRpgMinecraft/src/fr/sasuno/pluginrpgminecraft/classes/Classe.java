...
public abstract class Classe {
	...
	// Puisque c'est une classe abstraite (classe m�re), les constructeurs sont dans les classes h�rit�es (classes filles)
	public Classe() {}
	
	// Toutes fonctions relativent � l'objet de type Classe
	...
	public static void setClasse(Player player, String classeName) {
		...
		// On met � jour les infos du joueur
	...
	// Toutes fonctions relativent � la vie
	...
	protected static void cooldown(Player player) {
		...
		Thread cooldown = new Thread(new Runnable() {
			...
			public void run() {
				...
				changeOnCooldown(player);
				player.sendMessage("�aVotre pouvoir est recharg�");				
			}
	...
	public static boolean hasItemMenu(Player player) {
		...
		menuClasseMeta.setDisplayName("�bPierre mystique d'am�lioration");
		menuClasseMeta.setLore(Arrays.asList("Cette pierre magique permet", "d'am�liorer vos comp�tences", "", "�9Tout aventurier en a une sur lui", "�cNe perdez pas cet item"));
